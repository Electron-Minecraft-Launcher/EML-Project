...
function updateNavigationButtons(currentPageId) {
  // 1. Nettoyer les anciens boutons
  const existingNav = contentElement.querySelector('.doc-nav');
  if (existingNav) {
    existingNav.remove();
  }

  // 2. Trouver l'index actuel dans la liste des liens
  const links = Array.from(sidebarLinks);
  const currentIndex = links.findIndex(link => link.href.endsWith(`#${currentPageId}`));

  if (currentIndex === -1) {
    return; // Page non trouvée dans la nav, ne rien faire
  }

  // 3. Trouver les liens précédent et suivant
  ...
  // 7. Ajouter le conteneur à la page (s'il n'est pas vide)
