...
| `profile`                              | `IProfile`                                                              | The profile to launch. Retrieve the list with [`Profiles.getProfiles()`](/docs/v2/eml-lib-and-launcher/api-reference/profile). In agnostic mode (no `url`), the profile must have a valid `slug` for folder naming.                                                                        | No                                                |
...
| `profile.minecraft.loader.loader`      | `'vanilla' &#124 'forge' &#124 'neoforge' &#124 'fabric' &#124 'quilt'` | The mod loader to use for this profile.                                                                                                                                                                                                         | No (defaults to `'vanilla'`)                      |
| `profile.minecraft.loader.version`     | `string`                                                                | The mod loader version. Required for any loader other than `'vanilla'`.                                                                                                                                                                         | No                                                |
| `profile.minecraft.loader.manifestUrl` | `string`                                                                | Direct URL to a custom `version.json` (version manifest) for a custom version of the selected loader.                                                                                                                                           | No                                                |
...
| `minecraft.loader.loader`              | `'vanilla' &#124 'forge' &#124 'neoforge' &#124 'fabric' &#124 'quilt'` | The global mod loader type.                                                                                                                                                                                                                     | No (defaults to `'vanilla'`)                      |
| `minecraft.loader.version`             | `string`                                                                | The global mod loader version. Required for any loader other than `'vanilla'`.                                                                                                                                                                  | No                                                |
| `minecraft.loader.manifestUrl`         | `string`                                                                | Direct URL to a custom `version.json` (version manifest) for a custom version of the selected loader.                                                                                                                                           | No                                                |
...
| `storage`                              | `'isolated' &#124 'shared'`                                             | How game files are organised on disk. `'isolated'` gives each profile its own folder. `'shared'` shares assets and libraries between profiles while keeping mods, config, and saves separate. If `'shared'`, set `cleaning.enabled` to `false`. | No (defaults to `'isolated'`)                     |
...
| `java.install`                         | `'auto' &#124 'manual'`                                                 | `'auto'` downloads and manages the correct Java version automatically. `'manual'` uses a pre-installed Java executable.                                                                                                                         | No (defaults to `'auto'`)                         |
...
You can use a custom `version.json` (version manifest) to distribute a modified version of any loader (using MCP, Forge Gradle, etc.). This is useful for modpacks that require a specific loader version or have custom libraries.

To use a custom loader, you are recommended to use EML AdminTool (see [Files Updater](/docs/v2/eml-admintool/administration-and-features/files-updater#mod-loader-configuration)). 

If you are not using EML AdminTool, you can manually set the `profile.minecraft.loader.manifestUrl` (or `minecraft.loader.manifestUrl` property) to point to your custom `version.json`. Ensure that the `version.json` is valid, based on the selected loader, and that all libraries and assets referenced in it are accessible via HTTP(S) URLs.

> [!NOTE]
> You can set a custom `assetIndex`, even in modded loaders, by including the `assetIndex` property in your custom `version.json`. This asset index will override the default Vanilla asset index for that version, and EML Lib will download it automatically. If not using EML AdminTool, ensure that the `assetIndex.json` is valid and that all assets referenced in it are accessible via HTTP(S) URLs: in your `assetIndex.json`, add the `url` property to each custom asset, so EML Lib can download it. 
