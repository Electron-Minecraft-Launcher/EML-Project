...
When prompted for a domain name, you can enter the one you will use to access EML AdminTool (for example, `at.emlproject.com`), without `http://` or `https://`. This is useful if you plan to configure HTTPS later using a reverse proxy (see [Configuring NGINX](/docs/set-up-nginx)). If you do not have a domain name, simply press Enter to skip this step.
