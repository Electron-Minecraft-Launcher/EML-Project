...
> [!WARNING]
> Bootstraps requires an [EML AdminTool](/docs/install-eml-admintool) website to work.
...
async function bootstraps() {
  const bootstraps = new Bootstraps('https://at.emlproject.com')
...
| Parameter | Type      | Description                                       | Required?               |
| --------- | --------- | ------------------------------------------------- | ----------------------- |
| `silent`  | `boolean` | (Windows-only) Runs the installer in silent mode. | No (defaults to `false`) |
