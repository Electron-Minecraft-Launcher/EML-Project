...
> [!WARNING]
> **Domain Requirement:**
> You **must** have a valid domain name (e.g., `at.emlproject.com`) pointing to your server's public IP.
...
3. **Select Domains:** Certbot will list the domains found in your `server_name` directive (from the previous step).
   * **Select ONLY your domain name(s)** (e.g., `at.emlproject.com`).
...
Your EML AdminTool is now secure and accessible via `https://at.emlproject.com`.